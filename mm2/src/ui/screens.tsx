...
export const CreateScreen: React.FC<{ apply: Apply }> = ({ apply }) => {
  const [chars, setChars] = useState<Character[]>(() =>
    [0, 1, 2, 3, 4, 5].map(i => E.makeCharacter(
      i, E.NAMES[i],
      ['human', 'dwarf', 'elf', 'human', 'half-orc', 'gnome'][i],
      ['knight', 'paladin', 'sorcerer', 'cleric', 'barbarian', 'ninja'][i],
    ))
  );
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <p className="text-center text-mm-light/50 text-sm mb-4">挑選六位冒險者的種族與職業，擲骰決定能力值。（之後可在中央之門酒館雇用最多 2 名幫手）</p>
...
// ============ Hire hirelings ============
export const HireScreen: React.FC<{ g: GameState; apply: Apply }> = ({ g, apply }) => {
  return (
    <div className="w-full max-w-xl">
      <div className="flex items-center mb-3">
        <UserPlus className="text-mm-gold mr-2" />
        <h2 className="font-rune text-xl text-mm-gold">雇用幫手</h2>
        <span className="ml-auto text-mm-gold flex items-center gap-1"><Coins size={14} />{g.gold}</span>
      </div>
      <p className="text-mm-light/50 text-xs mb-3">隊伍最多 8 人（6 名冒險者 + 最多 2 名幫手）。目前 {g.party.length}/8。</p>
      <Panel className="space-y-1">
        {HIRELINGS.map(h => {
          const hired = !!g.flags[`hired_${h.id}`];
          const full = g.party.length >= 8;
          return (
            <div key={h.id} className="flex items-center gap-2 text-sm py-1.5 border-b border-mm-edge/40">
              <div className="flex-1">
                <span className="text-mm-light font-bold">{h.name}</span>
                <span className="text-mm-light/50 text-xs ml-1">{raceMap[h.raceId].name}{classMap[h.classId].name} · L{h.level}</span>
              </div>
