...
export function makeCharacter(id: number, name: string, raceId: string, classId: string): Character {
  ...
  const gear: Record<string, Partial<Record<EquipSlot, string>>> = {
    ...
    ninja: { weapon: 'katana', armor: 'leather' },
    barbarian: { weapon: 'battle_axe', armor: 'leather' },
  };
...
export function critChance(ch: Character): number {
  ...
  const base = cls.id === 'ninja' ? 0.32 : cls.id === 'robber' ? 0.25 : 0.05;
  return base + luck;
}
...
export function startAdventure(g: GameState, party: Character[]) {
  g.party = party;
  g.screen = 'town';
  pushLog(g, '你們抵達了中央之門 Middlegate。');
}

// ---------- hirelings (up to 2 beyond a party of 6, for 8 total) ----------
export const MAX_PARTY = 8;
export function recruitHireling(g: GameState, h: { id: string; name: string; raceId: string; classId: string; level: number; cost: number }): boolean {
  if (g.flags[`hired_${h.id}`]) { toast(g, '已經雇用過了'); return false; }
  if (g.party.length >= MAX_PARTY) { toast(g, '隊伍已滿（最多 8 人）'); return false; }
  if (g.gold < h.cost) { toast(g, '金幣不足'); return false; }
  g.gold -= h.cost;
  const ch = makeCharacter(Date.now() % 100000, h.name, h.raceId, h.classId);
  ch.level = h.level;
  recompute(ch);
  ch.hp = ch.maxHp;
  ch.sp = ch.maxSp;
...
export function castOutside(g: GameState, casterIdx: number, spellId: string, targetIdx: number): boolean {
  ...
  if (sp.kind === 'heal') {
    ...
    if (sp.id === 'wizard_eye') {
      g.flags['wizard_eye'] = true;
      pushLog(g, `👁 ${caster.name} 施放了巫師之眼，自動地圖已揭示。`);
    } else {
      g.flags['light'] = true;
